...
# alert-notification-service Kubernetes 배포 스크립트

set -e

NAMESPACE="flet-montrg"
SERVICE_NAME="alert-notification-service"
IMAGE_NAME="flet-montrg/alert-notification-service:latest"
KIND_CLUSTER="flet-cluster"

echo "🚀 Alert Notification Service 배포 시작..."
...
if [ "$1" != "--no-build" ] && [ "$2" != "--no-build" ]; then
    echo "🔨 Docker 이미지 빌드..."
    cd ../../services/alert-notification-service
    docker build -t $IMAGE_NAME .
    cd ../../k8s/alert-notification
fi
...
# 접속 정보 표시
echo ""
echo "✅ Alert Notification Service 배포 완료!"
echo "🔗 접속 정보:"
echo "  📊 API Endpoint: http://localhost:30009"
echo "  📖 API Docs: http://localhost:30009/docs"
echo "  💾 메트릭: http://localhost:30238/metrics"
echo ""
echo "📋 유용한 명령어:"
