...
<div class="max-w-[min(1200px,100%)]">
  <div class="mb-8">
    <h1
      class="m-0 mb-1 text-[1.625rem] font-bold tracking-tight"
    ...
    <p class="m-0 text-[0.85rem]" style="color: var(--text-tertiary);">
      {$serviceCount}
      {L.servicesIntegrated}
    </p>
  </div>
  ...
  <section class="mb-10">
    <h2
      class="m-0 mb-4 text-[1.125rem] font-bold tracking-tight"
    ...
    <div class="grid gap-4 grid-cols-[repeat(auto-fill,minmax(140px,1fr))]">
      ...
      >
        ...
        <div class="flex flex-col items-center gap-0.5 text-center">
          <span class="text-[0.8rem] font-medium leading-snug" style="color: var(--text-primary);"
            >{L.swaggerUI}</span
          >
          <span class="text-[0.7rem] leading-snug" style="color: var(--text-tertiary);"
            >{L.swaggerUiCardDesc}</span
          >
        </div>
      </a>
    </div>
  ...
  <section>
    <div class="flex flex-wrap items-start justify-between gap-4 mb-4">
      <div>
        <h2
          class="m-0 mb-1 text-[1.125rem] font-bold tracking-tight"
          style="color: var(--text-primary);"
        >
          {L.tableApis}
        </h2>
        <p class="m-0 mb-4 text-[0.9rem] leading-normal" style="color: var(--text-secondary);">
          {L.tableApisDesc}
        </p>
      </div>
      <div class="flex items-center gap-2">
        <div
          class="flex items-center min-w-[200px] py-2 px-3 rounded-lg border"
    ...
    {#if $filteredServiceNames.length === 0}
      <p class="py-8 text-center text-[0.95rem]" style="color: var(--text-tertiary);">
        {L.noServices}
      </p>
    {:else}
      <div class="grid gap-4 grid-cols-[repeat(auto-fill,minmax(140px,1fr))]">
        {#each $filteredServiceNames as name}
          ...
          >
            ...
            <div class="flex flex-col items-center gap-0.5 text-center">
              <span
                class="text-[0.8rem] font-medium leading-snug"
