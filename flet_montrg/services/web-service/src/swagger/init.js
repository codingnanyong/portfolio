...
export function initSwaggerUI(specUrl, _title, onComplete, onFailure, localeKey = "en") {
  ...
  const url = specUrl.startsWith("http")
    ? specUrl
    : base + (specUrl.startsWith("/") ? specUrl : "/" + specUrl);
  ...
  function hideInfoAndAuth() {
    ...
    root.querySelectorAll(".wrapper, section").forEach((el) => {
      if (
        el.querySelector(".auth-wrapper, .auth-container, .information-container, .info") &&
        !el.querySelector(".opblock-tag-section")
      ) {
        el.style.setProperty("display", "none", "important");
      }
    });
