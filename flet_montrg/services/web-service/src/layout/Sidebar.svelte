<script>
  ...
  function navLinkClass(isActive) {
    return (
      "flex items-center justify-between py-2 px-3 my-0.5 rounded-md text-[0.9rem] transition-colors " +
      (isActive
        ? "font-semibold border-l-[3px] pl-[calc(0.75rem-3px)] -ml-px bg-[var(--accent-muted)] border-l-[var(--accent)]"
        : "")
    );
  }
...
>
  <a
    href="#overview"
    class="flex items-center gap-3 py-6 px-5 border-b no-underline hover:no-underline"
    style="border-color: var(--border); color: inherit;"
  >
    <span
      class="w-7 h-7 flex-shrink-0 flex items-center justify-center rounded-full overflow-hidden"
    ...
    >
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 28 28" class="w-full h-full">
        ...
        <rect
          ...
          stroke-width="1.2"
        ></rect>
        <circle cx="14" cy="19.5" r="2.2" fill="none" stroke="white" stroke-width="1.2"></circle>
        <!-- 수은주: 빨간색 -->
        <rect x="12.7" y="9" width="2.6" height="6" rx="0.4" fill="#e11d48"></rect>
        <circle cx="14" cy="19.5" r="1.2" fill="#e11d48"></circle>
      </svg>
    </span>
    <div class="min-w-0">
      <h1
        class="m-0 text-[1.35rem] font-extrabold leading-tight tracking-tight"
        style="color: var(--text-primary); letter-spacing: -0.03em;"
      >
        {APP_NAME}
      </h1>
      <span class="block mt-1 text-[0.8125rem] font-medium" style="color: var(--text-secondary);"
        >{APP_SUBTITLE}</span
      >
    </div>
  </a>
  ...
  <nav class="flex flex-col flex-1 min-h-0 overflow-hidden py-4 px-2.5">
    <div class="flex-shrink-0 mb-4 py-2">
      <span
        class="block pb-2 mb-2 text-[0.6875rem] font-bold uppercase tracking-wider border-b"
        style="color: var(--text-secondary); border-color: var(--border);">{L.quickLinks}</span
      >
      <a
        href="#overview"
    ...
    <div class="flex flex-col flex-1 min-h-0 py-2 overflow-hidden">
      <span
        class="block pb-2 mb-2 text-[0.6875rem] font-bold uppercase tracking-wider border-b flex-shrink-0"
        style="color: var(--text-secondary); border-color: var(--border);">{L.tableApis}</span
      >
      <div class="serviceListScroll flex flex-col gap-px flex-1 min-h-0 overflow-y-auto mt-1">
        {#each serviceList as name}
          ...
          <a
            href="#swagger"
            data-service="{name}"
            class="{navLinkClass(isActive)} text-[0.875rem] hover:bg-[var(--bg-tertiary)] {spec &&
            !spec.is_available
